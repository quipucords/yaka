...
def prepare_crate_as_vendored_dep(crate_path: Path):
    ...
    Extracts the crate's contents and adds a ".cargo_checksum.json" file to make it compatible
    with Cargo's vendoring requirements.
    """
...
def prepare_cargo_config(crates_path):
    ...
    config_path = Path("/tmp/.cargo/config.toml")
